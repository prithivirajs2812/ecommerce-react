    ...
    <div className="max-w-4xl mx-auto px-6 py-10">
      ...
      <div className="grid md:grid-cols-3 gap-10">
        <div className="md:col-span-2 space-y-8">
          <section>
            <h2 className="font-semibold text-gray-800 text-lg mb-3">Shipping Address</h2>
            <div className="space-y-3">
              {addresses.map((addr) => (
                <label
                  key={addr.id}
                  className={`flex items-start gap-3 border rounded-xl p-4 cursor-pointer transition-all duration-200 hover:scale-[1.01] ${
                    selectedAddressId === addr.id ? 'border-brand-pink bg-pink-50/50 shadow-sm' : 'border-gray-200'
                  }`}
                >
                  <input
                    type="radio"
              ...
              {showNewAddressForm ? (
                ...
                <button
                  ...
                  className="text-base text-brand-pink font-semibold hover:underline hover:translate-x-1 transition-transform inline-block"
                >
                  + Add a new address
                </button>
              )}
          ...
          <section>
            <h2 className="font-semibold text-gray-800 text-lg mb-3">Payment Method</h2>
            <div className="flex gap-3 flex-wrap">
              {['COD', 'CARD', 'UPI'].map((method) => (
                <button
                  key={method}
                  onClick={() => setPaymentMethod(method)}
                  className={`press px-5 py-2.5 rounded-xl text-base font-medium border transition-all duration-200 hover:scale-105 ${
                    paymentMethod === method
                      ? 'border-brand-pink bg-pink-50/50 text-brand-pink shadow-sm'
          ...
          <section>
            <h2 className="font-semibold text-gray-800 text-lg mb-3">Coupon Code</h2>
            <input
              value={couponCode}
              onChange={(e) => setCouponCode(e.target.value)}
              placeholder="Enter coupon code (optional)"
              className="w-full max-w-xs border border-gray-300 rounded-xl px-4 py-3 text-base focus:outline-none focus:ring-2 focus:ring-brand-pink transition-shadow"
            />
          </section>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm p-6 h-fit">
          ...
          <button
            ...
            className="press w-full mt-6 bg-brand-pink hover:bg-pink-600 hover:scale-[1.02] disabled:opacity-60 disabled:hover:scale-100 transition-all duration-200 text-white text-lg font-semibold py-4 rounded-xl shadow-md"
          >
